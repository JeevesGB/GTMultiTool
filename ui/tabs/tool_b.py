...
def load_tim(filepath):
    with open(filepath, 'rb') as f:
        ...
        expected_size = w16 * 2 * h
        if len(img) < expected_size:
            raise ValueError(f"Image data too short: expected {expected_size} bytes, got {len(img)}")
        elif len(img) > expected_size:
...
class ToolBTab(QWidget):
    ...
    def _build_ui(self):
        ...
        btn_backup = QPushButton("Backup TIM")
        btn_backup.clicked.connect(self.backup_tim)
        btn_save = QPushButton("Save TIM")
        btn_save.clicked.connect(self.save_tim)
        btn_frame.addWidget(btn_backup)
        btn_frame.addWidget(btn_save)
        self.right.addLayout(btn_frame)
    ...
    def backup_tim(self):
        if not self.image_info or "path" not in self.image_info:
            QMessageBox.warning(self, "Backup TIM","No TIM loaded!")
            return
        path = self.image_info["path"]
        folder = os.path.join(os.path.dirname(path),"backup")
        os.makedirs(folder, exist_ok=True)
        backup_path = os.path.join(folder, os.path.basename(path))
        try:
            with open(path,'rb') as src, open(backup_path,'wb') as dst:
                dst.write(src.read())
            QMessageBox.information(self,"Backup TIM",f"Backup saved:\n{backup_path}")
        except Exception as e:
            QMessageBox.critical(self,"Backup TIM",str(e))

    def save_tim(self):
        if not self.image_info or "path" not in self.image_info:
            QMessageBox.warning(self, "Save TIM","No TIM loaded!")
            return
        path = self.image_info["path"]
        try:
            with open(path,'wb') as f:
                ...
                if self.image_info["clut"]:
                    w = self.image_info["clut_w"]
                    h = self.image_info["clut_h"]
                    clut_len = 12 + w*h*2
                    f.write(struct.pack('<I',clut_len))
                    f.write(struct.pack('<HHHH',0,0,w,h))
                    for r,g,b in self.image_info["clut"]:
                        val = ((b>>3)<<10)|((g>>3)<<5)|(r>>3)
                        f.write(struct.pack('<H',val))

                data = self.image_info["data"]
                h = self.image_info["height"]
                w16 = self.image_info["w16"]
                bpp = self.image_info["bpp"]

                if bpp in [4,8,16]:
                    img_len = 12 + w16 * 2 * h
                else:
                    img_len = 12 + self.image_info["width"] * 3 * h

                f.write(struct.pack('<I', img_len))
                f.write(struct.pack('<HHHH', 0,0,w16,h))
                ...
                if bpp == 4:
                    for row in data:
                        for i in range(0,len(row),4):
                            ...
                            for j in range(4):
                                if i+j<len(row):
                                    word |= (row[i+j]&0xF)<<(4*j)
                            f.write(struct.pack('<H',word))
                elif bpp == 8:
                    for row in data:
                        for i in range(0,len(row),2):
                            b0 = row[i]
                            b1 = row[i+1] if i+1<len(row) else 0
                            f.write(struct.pack('<H', b0|(b1<<8)))
                elif bpp == 16:
                    for row in data:
                        for r,g,b in row:
                            val = ((b>>3)<<10)|((g>>3)<<5)|(r>>3)
    ...
    def get_state(self):
        return {
            ...
            "last_tim": self.image_info["path"] if getattr(self, "image_info", None) else "",
            "palette": self.image_info.get("clut") if getattr(self, "image_info", None) else None,
    ...
    def load_state(self, state):
        ...
        if palette and getattr(self, "image_info", None):
